...
const DailyDietSuggestion = ({ imcCategory }: DailyDietSuggestionProps) => {
  ...
  const timeConfig = [
    { key: 'morning' as const, label: 'Manhã', icon: Sun, color: 'text-warning', bgColor: 'bg-warning/10' },
    { key: 'afternoon' as const, label: 'Tarde', icon: CloudSun, color: 'text-primary', bgColor: 'bg-primary/10' },
    { key: 'night' as const, label: 'Noite', icon: Moon, color: 'text-info', bgColor: 'bg-info/10' },
  ];
      ...
      <div className="space-y-4">
              ...
              return (
                ...
                >
                  <div className="absolute inset-0">
                    <img src={recipeImage} alt={recipe?.name || 'Receita'} className="w-full h-full object-cover" loading="lazy" />
                  ...
                  <div className="relative flex items-center gap-3 p-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-medium text-white truncate">{recipe?.name || 'Não disponível'}</p>
                      {recipe?.calories && <p className="text-xs text-white/70">{recipe.calories} kcal</p>}
                    </div>
        ...
        {dailySelection.dessert && (
          <div>
            ...
            >
              <div className="absolute inset-0">
                <img src={getRecipeImage('dessert', imcCategory, 0)} alt={dailySelection.dessert.name} className="w-full h-full object-cover" loading="lazy" />
                <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/50 to-transparent" />
              </div>
              <div className="relative flex items-center gap-3 p-3">
                <div className="w-8 h-8 rounded-lg bg-white/20 backdrop-blur flex items-center justify-center">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm font-medium text-white truncate">{dailySelection.dessert.name}</p>
                  {dailySelection.dessert.calories && <p className="text-xs text-white/70">{dailySelection.dessert.calories} kcal</p>}
                </div>
                <ChevronRight className="w-4 h-4 text-white/70 flex-shrink-0" />
              </div>
            </motion.div>
          ...
          <div className="space-y-2">
            {timeConfig.map((time, index) => {
              ...
              return (
                ...
                >
                  <div className="absolute inset-0">
                    <img src={detoxImage} alt={drink?.name || 'Detox'} className="w-full h-full object-cover" loading="lazy" />
                    <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/50 to-transparent" />
                  </div>
                  <div className="relative flex items-center gap-3 p-3">
                    <div className="w-8 h-8 rounded-lg bg-white/20 backdrop-blur flex items-center justify-center">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-white/80">{time.label}</p>
                      <p className="text-sm font-medium text-white truncate">{drink?.name || 'Não disponível'}</p>
                    </div>
                    <ChevronRight className="w-4 h-4 text-white/70 flex-shrink-0" />
                  </div>
                </motion.div>
      ...
      <Dialog open={!!selectedDrink} onOpenChange={() => setSelectedDrink(null)}>
        <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
          {selectedDrink && (
            <>
              <div className="relative h-40 -mx-6 -mt-6 mb-4 overflow-hidden rounded-t-lg">
                <img src={getDetoxImage(selectedDrink.timeOfDay, 0)} alt={selectedDrink.name} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                <div className="absolute bottom-4 left-4 right-4">
